...
function resta_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'resta' ),
        'next_string'     => esc_html__( '&raquo;', 'resta' ),
        'before_output'   => '<div class="resta-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'resta_pagination_defaults', $defaults )
    );
