...
function resta_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'resta' ) . '</span></li>';
...
// Blog categories
function resta_featured_post_cat(){
...
// social media
if ( ! function_exists( 'resta_social' ) ) {
	function resta_social( $args = array() ) {
		...
		$url = resta_opt( 'resta_social_url' );
...
// header cart count
function resta_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( RESTA_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'resta' ); ?>">
...
function resta_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'resta_contact7_form_content', 10, 2 );