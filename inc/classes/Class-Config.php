...
final class Resta {
	...
	private function setup() {
		...
		$enqueu          = new resta_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->resta_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'resta_content_width', 751 );


		// text domain for translation.
		load_theme_textdomain( 'resta', RESTA_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'resta_site_logo_189x100', 189, 100, true );
		add_image_size( 'resta_site_footer_logo_146x52', 146, 52, true );

		// About thumb sizes
		add_image_size( 'resta_about_1_thumb_490x469', 490, 469, true );
		add_image_size( 'resta_about_2_thumb_351x324', 351, 324, true );

		// Menu thumbnail size
		add_image_size( 'resta_menu_thumb_166x166', 166, 166, true );

		// Latest post thumbnail size
		add_image_size( 'resta_widget_post_thumb', 80, 80, true );

		// Single blog post image size
		add_image_size( 'resta_single_blog_750x375', 750, 375, true );
		add_image_size( 'resta_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu'	=> esc_html__( 'Primary Menu', 'resta' ),
			'services'		=> esc_html__( 'Services', 'resta' ),
			'company' 		=> esc_html__( 'Company', 'resta' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = RESTA_DIR_CSS_URI;
		$jsPath  = RESTA_DIR_JS_URI;
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'resta' ) ) {
	...
	private function customizer_init(){
		...
		// Instantiate resta theme customizer
		$resta_theme_customizer = new resta_theme_customizer();
	}
	...
	public function set_elementor_flag(){
		update_option( 'resta_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'resta-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'resta_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'resta-elementor-default-style',  RESTA_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->resta_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'resta_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'resta-elementor-notice', RESTA_DIR_JS_URI.'resta-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'resta-elementor-notice',
				'restaElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'resta-elementor-notice-nonce' ),
				)
			);
		}
